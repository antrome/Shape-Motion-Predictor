...
def fkl( angles, parent, offset, rotInd, expmapInd ):
  ...
  # Structure that indicaes parents for each joint
  njoints   = 32
...
def main():
  ...
  with h5py.File(os.path.join("/home/finuxs/Shape-Motion-Predictor/datasets/h36m/","h36trainall.hdf5"), 'r' ) as h5f:
    expmap_gt = h5f['000'].value
    print(expmap_gt)
    expmap_pred = h5f['001'].value
    #expmap_gt = h5f['expmap/gt/walking_0'][:]
    #expmap_pred = h5f['expmap/preds/walking_0'][:]

  #nframes_gt, nframes_pred = expmap_gt.shape, expmap_pred.shape
